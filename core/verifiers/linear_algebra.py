...
class LinearAlgebraVerifier(BaseVerifier):
    ...
    def can_handle(self, topic, enonce, solution, final_answer, check):
        t = (topic or "").lower()
        return any(k in t for k in ["matrice", "matrix", "déterminant", "determinant", "vecteur", "espace"]) and (final_answer is not None)
    ...
    def verify(self, topic, enonce, solution, final_answer, check):
        ...
        # If CHECK is Eq(det(A), value) or a Matrix expression, verify
        if check:
            try:
                ck = sp.sympify(check)
